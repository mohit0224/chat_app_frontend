...
export default function RootLayout({ children }) {
	...
	return (
		<>
			...
			<div className="h-screen w-screen fixed top-0 left-0 right-0 pt-20 overflow-y-scroll">
				<div className="h-full flex">
					<div
						className={`lg:block w-full lg:w-3/12 h-full overflow-y-scroll border-r p-5 space-y-3
								${isChatOpen && "hidden"}
							`}
					>
						<SearchUser />
					...
					<div
						className={`h-full w-full flex-1 lg:block
							${!isChatOpen && "hidden"}
						`}
					>
						{children}
